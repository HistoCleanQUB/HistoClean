...
HomeScreen.iconphoto(False, tk.PhotoImage(file=r"Icon/HistoClean Icon.png"))
...
def Patch_App():
    ...
    ListList = [InputImages, OriginalWidthList, OriginalHeightList, ImageLocationXList, ImageLocationYList, DownfactorList,
    ...
    Patch_Interface.iconphoto(False, tk.PhotoImage(file=r"Icon/HistoClean Icon.png"))
    ...
    def LoadImages():
        ...
        for r, d, f in os.walk(Step1Entry.get()):
            for file in f:
                ...
                if 'jpg'in extension or 'jfif'in extension or "png" in extension:
                    print(file)
    ...
    def PreviewGrid():
        ...
        for x in range(0, XRectanglesNeeded, 1):
            ...
            for y in range(0, YRectanglesNeeded, 1):
                ...
                Rect = PreviewCanvas.create_rectangle(int(TopLeftX), int(TopLeftY), int(BottomRightX), int(BottomRightY), width=2, outline="red")
                DrawnLines.append(Rect)

        if XRectanglesNeeded >= 1 and YRectanglesNeeded >= 1:
            Line = PreviewCanvas.create_line(int(ImageLocationXList[CurrentImage]), int(ImageLocationYList[CurrentImage]+2), int(BottomRightX), int(ImageLocationYList[CurrentImage]+2), width=4, fill="red")
            DrawnLines.append(Line)
            ImageSizeLabel.config(fg="blue", text=f"Image Size: {OriginalWidthList[CurrentImage]} X {OriginalHeightList[CurrentImage]} pixels")
        else:
            ImageSizeLabel.config(fg="red", text=f"Image Size: {OriginalWidthList[CurrentImage]} X {OriginalHeightList[CurrentImage]} pixels. Image Too Small to produce tiles of this size")
    ...
    def NextImage():
        ...
        if CurrentImage < len(InputImages)-1:
            CurrentImage += 1
        PreviewCanvas.delete(tk.ALL)
        PreviewCanvas.create_image(CanvasWidth / 2, CanvasHeight / 2, image=ThumbnailImages[CurrentImage], anchor="center")
        ImageSizeLabel.config(fg="blue", text=f"Image Size: {OriginalWidthList[CurrentImage]} X {OriginalHeightList[CurrentImage]} pixels")
        PreviewGrid()
    ...
    def Pop():
        ...
        Popup.iconphoto(False, tk.PhotoImage(file=r"Icon/HistoClean Icon.png"))
    ...
    WidthEntry = tk.Entry(Patch_Interface, width=5, justify="center", validate="key", validatecommand=(Validation, "%P", "%d"))
    ...
    WarningLabel = tk.Label(Patch_Interface, text="WARNING - Save directory is not empty. Images may not save correctly.", fg="red")
...
def White_App():
    ...
    WS_Interface.iconphoto(False, tk.PhotoImage(file=r"Icon/HistoClean Icon.png"))
    ...
    def BringUpOptions():
        ...
        if TypeDropdown.get() == "Positive Pixel Classification":
            ...
            Info_Label.config(text="Uses an algorithm to calculate thresholds for small regions of the image (Block size).\n"
            ...
            if TypeDropdown.get() == "Positive Pixel Classification":
                ...
                for I in range(len(ShownImageThumbnailMasks)):
                    ...
                    MaskCanvas.create_text(CanvasXLoc + 60, 250,
                                           text=f"Tissue: {str(round(ShownImagePositivePercentage[I], 2))}%", anchor="w")
                    CanvasXLoc += (width + DeadspaceFraction)
                ...
                for I in ShownImagesPaths:
                    ...
                    outimg = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, BlockSize, Anchor)
                    width, height = outimg.shape
                    ImageArea = (width * height) * 255
                    PositivePixels = np.sum(outimg)
                    PositivePercentage = 100 - ((PositivePixels / ImageArea) * 100)
                ...
                for I in ShownImagesPaths:
                    ...
                    outimg = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, BlockSize,
                                                   Anchor)
                    width, height = outimg.shape
                    ImageArea = (width * height) * 255
                    PositivePixels = np.sum(outimg)
    ...
    EV = tk.StringVar()
    Step2Entry = tk.Entry(WS_Interface, width=5, textvariable=EV, justify="center", validate="key", validatecommand=(Validation, "%P", "%d"))
    Step2Entry.place(anchor="center", relx=0.17, rely=0.2)
    Step2Entry.bind_all('<Key>', Change_Slider)

    Step2Scale = tk.Scale(WS_Interface, from_=0, to=100, orient="horizontal", showvalue=0, tickinterval=50, command=Change_Tissue_Entry)
    ...
    OptionList = ["Positive Pixel Classification", "Adaptive Threshold - Mean", "Adaptive Threshold - Gaussian", "Otsu Binarisation"]
    ...
    KeepChecker = tk.Checkbutton(WS_Interface, variable=CheckVar, offvalue=0, onvalue=1, text="Keep sub-threshold images in separate folder?")
...
def Balance_App():
    ...
    Balance_Interface.iconphoto(False, tk.PhotoImage(file=r"Icon/HistoClean Icon.png"))
    ...
    SetCanvas = tk.Canvas(SetFrameOutter, width=400, height=300, bg="gray86", scrollregion=(0, 0, 400, 300), highlightthickness=0)
    ...
    Bal_Avg = tk.Radiobutton(Balance_Interface, text="Balance to Average", variable=VarBal, value=2, command=BalanceInfo)
    ...
    pgbar = ttk.Progressbar(Balance_Interface, length=300, orient="horizontal", maximum=100, value=0, variable=ProgressVarB)
    pgbar.place(anchor="center", relx=0.5, rely=0.97)
...
def Normalisation_App():
    ...
    Norm_Interface.iconphoto(False, tk.PhotoImage(file=r"Icon/HistoClean Icon.png"))
    ...
    def NewSetOptions():
        ...
        if NormKeep_Var.get() == 1:
            NewSetLabel.place(anchor="center", relx=0.15, rely=0.8)
            NewSetEntry.place(anchor="center", relx=0.13, rely=0.85)
            NewSetButton.place(anchor="center", relx=0.23, rely=0.85)
    ...
    def GetImage():
        ...
        Target_Image = filedialog.askopenfilename(initialdir=os.getcwd(), title="Please select your top-level folder.",
                                                  filetypes=[("Image", "*.jpg"), ("Image", "*.png"), ("Image", "*.jfif")])
        ...
        for I in TargetImage:
            Norm_Image_Canvas.create_image(125, 125, image=I, anchor="center")
    ...
    def Norm_Threading():
        ...
        if len(ImagePaths) > 0:
            ...
            for I in range(len(ImagePaths)):
                #print(ImagePaths[I])
                pool.submit(NormCore, ImagePaths[I], TargetImage)
            pool.shutdown(wait=True)
    ...
    KeepImageCheck = tk.Checkbutton(Norm_Interface, variable=NormKeep_Var, offvalue=0, onvalue=1, text="Keep Original Images?",
                                    command=NewSetOptions)
    KeepImageCheck.place(anchor="center", relx=0.15, rely=0.75)
    ...
    pgbar = ttk.Progressbar(Norm_Interface, length=500, orient="horizontal", maximum=100, value=0, variable=ProgressVarN)
    pgbar.place(anchor="center", relx=0.65, rely=0.95)
    ...
    BeginButton = tk.Button(Norm_Interface, text="Begin Normalisation", height=1, width=15, command=Norm_Thread_Threading)
...
def Augmentation_App():
    ...
    def AddOption():
        ...
        AugLabel = tk.Label(AugFrameInner, text=f"Augmentation {AugOptions+1}", name=f"label{AugOptions}", bg="gray88")
        ...
        OptionButton = tk.Button(AugFrameInner, text="Options", name=f"options{AugOptions}", command=lambda: OpenOption(str(OptionButton)), bg="gray88")
        ...
        TypeDropdown = ttk.Combobox(AugFrameInner, values=OptionList, width=35, state="readonly", name=f"dropdown{AugOptions}")
    ...
    def DisplayColourOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "RGB Levels":
                ...
                if RedLevelVar.get() == 1:
                    AugList.append(iaa.WithChannels(0, iaa.Add((int(RedEntryMin.get()),int(RedEntryMin.get())), per_channel=1)))
                ...
                if GreenLevelVar.get() == 1:
                    AugList.append(iaa.WithChannels(1, iaa.Add((int(GreenEntryMin.get()), int(GreenEntryMax.get())),per_channel=1)))
                ...
                if BlueLevelVar.get() == 1:
                    AugList.append(iaa.WithChannels(2, iaa.Add((int(GreenEntryMin.get()), int(GreenEntryMax.get())),per_channel=1)))
        ...
        def DisplaySubOptions():
            ...
            if Option == "Invert Colours":
                ...
                ActiveWidgetList.append(RedInvert), ActiveWidgetList.append(GreenInvert), ActiveWidgetList.append(BlueInvert)
                ...
                ActiveWidgetList.extend((RedLabelMin, RedLabelMax, GreenLabelMin, GreenLabelMax, BlueLabelMin, BlueLabelMax,
    ...
    def DisplayBriConOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Sigmoid Contrast":
                ...
                AugList.append(iaa.SigmoidContrast(gain=(int(SigConEntryMinGain.get()), int(SigConEntryMaxGain.get())), cutoff=(float(SigConEntryMinCutoff.get()), float(SigConEntryMaxCutoff.get()))))
                ImportantWidgetValues.extend((float(SigConEntryMinCutoff.get()), float(SigConEntryMaxCutoff.get()), int(SigConEntryMinGain.get()), int(SigConEntryMaxGain.get())))
        ...
        def DisplaySubOptions():
            ...
            if Option == "Brightness":
                ...
                ActiveWidgetList.extend((SigConLabelMinCutoff, SigConLabelMaxCutoff, SigConEntryMinCutoff, SigConEntryMaxCutoff,
    ...
    def DisplaySharpEmbOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Sharpness":
                ...
                ImportantWidgetValues.extend((float(SharpEntryMin.get()), float(SharpEntryMax.get()), float(AlphaEntryMin.get()), float(AlphaEntryMax.get())))
            ...
            if SubTypeDropdown.get() == "Embossing":
                ...
                ImportantWidgetValues.extend((float(EmbEntryMin.get()), float(EmbEntryMax.get()), float(AlphaEmbEntryMin.get()), float(AlphaEmbEntryMax.get())))
                try:
        ...
        SharpLabel = tk.Label(Frame, text="Enter sharpness levels between 0 and 1")
        ...
        AlphaLabel = tk.Label(Frame, text="Enter alpha levles between 0 and 2")
        ...
        EmbLabel = tk.Label(Frame, text="Enter emboss levels between 0 and 2")
        ...
        AlphaEmbLabel = tk.Label(Frame, text="Enter alpha levles between 0 and 2")
    ...
    def DisplayNoiseOptions(Number, Frame, LastSub):
        OptionsText.place_forget()
        No = Number
        NoiseOptions = ["Gaussian Noise", "Laplace Noise", "Poisson Noise", "Salt and Pepper", "Impulse Noise", "Jpeg compression",
            ...
            if SubTypeDropdown.get() == "Laplace Noise":
                ...
                if LapColourVar.get() == 1:
                    AugList.append(iaa.AdditiveLaplaceNoise(
                        scale=((float(NoiseLapEntryMin.get()) * 255, float(NoiseLapEntryMax.get()) * 255)), per_channel=True))
                else:
                    AugList.append(iaa.AdditiveLaplaceNoise(
                        scale=(float(NoiseLapEntryMin.get()) * 255, float(NoiseLapEntryMax.get()) * 255), per_channel=False))
    ...
    def DisplayDropoutOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Coarse Dropout":
                ...
                AugList.append(iaa.CoarseDropout((float(CorDropoutEntryMin.get()),float(CorDropoutEntryMax.get())),
                                                 size_percent=(float(CorDropoutSizeEntryMin.get()), float(CorDropoutSizeEntryMax.get()))))

                ImportantWidgetValues.extend((float(CorDropoutEntryMin.get()), float(CorDropoutEntryMax.get()), float(CorDropoutSizeEntryMin.get()), float(CorDropoutSizeEntryMax.get())))
        ...
        def DisplaySubOptions():
            ...
            if Option == "Cutout":
                SubDescriptionLabel.config(text="Cut out random areas of each image.\n entered values are the number of areas\n"
            ...
            if Option == "Dropout":
                SubDescriptionLabel.config(text="Drop a percentage of pixels from images.\n Enter values between 0 and 1")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
    ...
    def DisplayBlurOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Bilateral Blur":
                ...
                AugList.append(iaa.BilateralBlur(d=(int(BiBlurEntryMin.get()), int(BiBlurEntryMax.get())), sigma_color=(10, 250), sigma_space=(10, 250)))
            ...
            if SubTypeDropdown.get() == "Motion Blur":
                ...
                ImportantWidgetValues.extend((int(MotBlurEntryMin.get()), int(MotBlurEntryMax.get()), int(MotBlurDirEntryMin.get()), int(MotBlurDirEntryMax.get())))
        ...
        def DisplaySubOptions():
            ...
            if Option == "Gaussian Blur":
                SubDescriptionLabel.config(text="Applies a gaussian filter.\n Enter kernal size using whole numbers greater than 1.")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
            ...
            if Option == "Average Blur":
                SubDescriptionLabel.config(text="Average Blur over a neibhorhood of pixels.\n Enter whole positive values greater than 1")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
            ...
            if Option == "Motion Blur":
                SubDescriptionLabel.config(text="Simulate motion blur on an image.\n Adjust intensity: Insert positive whole values.")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
    ...
    def DisplayGeoOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Jigsaw":
                ...
                AugList.append(iaa.Jigsaw(nb_rows=random.randint(Jigrowmin, Jigrowmax), nb_cols=random.randint(Jigcolmin, Jigcolmax)))
        ...
        def DisplaySubOptions():
            ...
            if Option == "Scale":
                SubDescriptionLabel.config(text="Scale the image on the X and Y axis\n Enter values greater than 0 (1 keeps the image the same).")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
            ...
            if Option == "Translate":
                SubDescriptionLabel.config(text="Translate the image along the x and y axis a a percentage of the image size.\n Enter any value (0 is default).")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
            ...
            if Option == "Shear":
                SubDescriptionLabel.config(text="Add a shear (slanted) effect to the images.\n Enter values between 0 and 90")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
            ...
            if Option == "Piecewise Affine":
                SubDescriptionLabel.config(text="Causes random tranformations around local pixel clusters.\n Enter values between 0 and 0.5.")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
            ...
            if Option == "Elastic Transformation":
                SubDescriptionLabel.config(text="Displaces pixels around local clusters.\n Enter values between 0 and 10.")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
    ...
    def DisplayEdgeOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Canny Edge Detection":
                ...
                AugList.append(iaa.Canny(alpha=(AMin, AMax), colorizer=iaa.RandomColorsBinaryImageColorizer(color_true=255, color_false=0)))
    ...
    def DisplayColourSegOptions(Number, Frame, LastSub):
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Voronoi":
                ...
                points_sampler = iaa.RegularGridPointsSampler(n_rows=random.randint(VoMinRows,VoMaxRows), n_cols=random.randint(VoMinCols,VoMaxCols))
            ...
            if SubTypeDropdown.get() == "Regular Grid Voronoi":
                ...
                AugList.append(iaa.RegularGridVoronoi((random.randint(VoMinRows, VoMaxRows), random.randint(VoMinCols,VoMaxCols)), p_replace=1))
        ...
        def DisplaySubOptions():
            ...
            if Option == "Voronoi":
                SubDescriptionLabel.config(text="Averages Colours within a set grid of cells. \n  Enter whole positive numbers")
                SubDescriptionLabel.place_configure(rely=0.13, relx=0.5, anchor="center")
            ...
            if Option == "Regular Grid Voronoi":
                ...
                ActiveWidgetList.extend(
                    (RegColsLabelMin, RegColsLabelMax, RegRowsLabelMin, RegRowsLabelMax, RegRowsEntryMin, RegRowsEntryMax,
    ...
    def DisplayArtSegOptions(Number, Frame, LastSub):
        OptionsText.place_forget()
        No = Number
        ArtOptions = ["Cartoon", "Snowflakes",  "Cloudy", "Fog",  "Rain", "Frost"]
    ...
    Aug_Interface.iconphoto(False, tk.PhotoImage(file=r"Icon/HistoClean Icon.png"))
    ...
    Step1Text = tk.Label(Aug_Interface, fg="black", text="Step 1: Please select the images you want to augment:", font=20)
    ...
    AugCanvas = tk.Canvas(AugFrameOutter, width=500, height=300, bg="gray86", scrollregion=(0, 0, 500, 300), highlightthickness=0)
    ...
    SetVar = tk.IntVar()
    SetVar.set(1)
    SameSetRadio = tk.Radiobutton(Aug_Interface, text="Use Selected Images", variable=SetVar, value=1,
                               command=NewSetPlacement)
    SameSetRadio.place(anchor="center", relx=0.65, rely=0.8)
    NewSetRadio = tk.Radiobutton(Aug_Interface, text="Create New Image Set", variable=SetVar, value=2,
                               command=NewSetPlacement)
    NewSetRadio.place(anchor="center", relx=0.75, rely=0.8)
