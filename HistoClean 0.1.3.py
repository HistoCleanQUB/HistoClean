...
#os.add_dll_directory(r'C:\Users\mccom\PycharmProjects\Histoclean 2022\openslide-win64\openslide-win64-20171122\bin')
import openslide
        ...
        if os.path.isdir(filename2):
            ...
            for r, d, f in os.walk(Step1Entry.get()):
                for file in f:
                    ...
                    extension = extension[-1]
                    print(extension)
                    if 'svs' in extension.lower() or 'zvi' in extension.lower() or 'ndpi' in extension.lower():
...
def WhitespaceModule():
    ...
    def LoadImages():
        ...
        for r, d, f in os.walk(Step1Entry.get()):
            for file in f:
                if '.jpg' in file.lower() or '.jfif' in file.lower() or ".png" in file.lower() or ".tif" in file.lower():
...
def BalanceModule():
    ...
    def NameSet(ButtonNumber):
        ...
        for r, d, f in os.walk(SelectedEntry.get()):
            for file in f:
                if '.jpg' in file.lower() or '.jfif' in file.lower()  or ".png" in file.lower() :
...
def AugmentationModule():
        ...
        def ConfirmOptions():
            ...
            if SubTypeDropdown.get() == "Crop":
                ...
                if FixedCropVar.get() == 1:
                    ...
                    # Need to revese these, imgaug places in incorrect location
                    if "top" in Crop_Location:
                        Crop_Location = Crop_Location.replace("top", "bottom")
                    elif "bottom" in Crop_Location:
                        Crop_Location = Crop_Location.replace("bottom", "top")

                    if "left" in Crop_Location:
                        Crop_Location = Crop_Location.replace("left", "right")
                    elif "right" in Crop_Location:
                        Crop_Location = Crop_Location.replace("right", "left")
    ...
    def AugCore(ImagePath):
        ...
        # if single channel, converts to multichannel
        if len(Orig_Image.shape) == 2:
            Orig_Image = skimage.color.gray2rgb(Orig_Image)
